...
rna_genes_relabd <- rna_genes %>%
  ...
  mutate(species = recode(gene, !!!genes2taxa)) %>%
  gather(key = sample, value = counts, 1:20) %>%
  group_by(species, sample) %>%
  mutate(relabd = counts / sum(counts)) %>%
  dplyr::select(-counts) %>%
...
scfa_stats <- scfa %>%
  mutate(time = substr(as.character(time + 13), 1, 4)) %>%
  mutate(time = factor(time, levels = c("14.3", "12.6", "13.6", "14.6", "15.3", "15.6",
...
lm_results <- lm_results %>%
  mutate(across(starts_with("p"), ~ case_when(.x < 0.001 ~ "***",
                                              .x < 0.01 & .x >= 0.001 ~ "**",
                                              .x < 0.05 & .x >= 0.01 ~ "*",
                                              .x >= 0.05 ~ "")))
```
...
rna_hits_labels <- rna_glm %>%
  ...
  dplyr::select(`Dose 1\nHr +6`, `Dose 2\nHr -2`, `Dose 2\nHr +6`) %>%
  filter(`Dose 1\nHr +6` < 0.05 | `Dose 2\nHr -2` < 0.05 | `Dose 2\nHr +6` < 0.05) %>%
  mutate(across(starts_with("D"), ~ case_when(.x < 0.001 ~ "<sub>***</sub>", #hack for centering asterisks
...
levsun_df <- rna_genes_relabd %>%
  ...
  mutate(label = case_when(gene == 641002968 ~ "*Bacteroides caccae* ATCC 43185 (BC02726)",
  ...
  mutate(label = label_wrap_gen2(label))
...
levsun_stars <- glm_levsun %>%
  ...
  mutate(star = case_when(star < 0.001 ~ "***",
                           star < 0.01 & star >= 0.001 ~ "**",
                           star < 0.05 & star >= 0.01 ~ "*",
                           star >= 0.05 ~ "")) %>%
  filter(star != "") %>%
  arrange(label) %>%
  mutate(height = c(0.002, 0.0085, 0.005, 0.0082,
                    0.47, 0.25, 0.36,
                    0.18, 0.14, 0.1))
...
scfa_total_plot <- ggplot(data=scfa_total_summary, aes(x=time, y=mean)) +
  ...
  xlab("Time (day)") + ylab("Total SCFA concentration (mM)") + 
  geom_ribbon(aes(ymin=mean-se, ymax=mean+se), fill="gray", alpha = 0.5) +
...
scfa_each_plot <- ggplot(data=scfa_each_summary, aes(x=time, y=mean)) +
  ...
  xlab("Time (day)") + ylab("SCFA concentration (mM)") + 
  geom_ribbon(aes(ymin=mean-se, ymax=mean+se), fill="gray", alpha = 0.5) +
  geom_vline(xintercept = c(1.42, 2.42, 3.42, 4.42, 5.42), color="darkgray", linetype="dashed") +
  geom_line(color="red") +
...
gcms_hits_labels <- lm_results_gcms %>%
  ...
  mutate(metabolite = recode(metabolite, !!!metab_name_map)) %>%
  mutate(across(starts_with("D"), ~ case_when(.x < 0.001 ~ "<sub>***</sub>", # hack to center asterisk
                                              .x < 0.01 & .x >= 0.001 ~ "<sub>**</sub>",
                                              .x < 0.05 & .x >= 0.01 ~ "<sub>*</sub>",
                                              .x >= 0.05 ~ ""))) %>%
...
gcms_hits_effect <- gcms_hits_effect %>%
  mutate(metabolite = factor(metabolite, levels = rev(gcms_order))) %>%
...
fig2_middle <- cowplot::plot_grid(rna_hm, levsun_plot,
                                  ...
                                  rel_widths = c(2, 3))
fig2_bottom <- cowplot::plot_grid(scfa_total_plot, gcms_select_plot,
                                  labels = c("D", "E"), label_fontface = "plain",
                                  rel_widths = c(2, 3))
fig2 <- cowplot::plot_grid(select_taxa_plot, fig2_middle, fig2_bottom,
...
# since saving, I think something got massed up with the case_when creation of asterisks...
